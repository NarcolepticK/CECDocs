...
Result GetChangeStateEventHandle(Handle *outHandle) {
  ...
  if (-1 < ret) {
    handle = GetHandleFromCmdBuf((u32 *)&cmdBuf,3);
    *outHandle = handle;
    ret = cmdBuf[1];
  }
  return ret;
...
Result WriteMessageWithHmac(u32 programId, bool isOutbox, u32 messageIdAddress, u32 messageIdSize,
                            u32 *bufferAddress, u32 *bufferSize, u32 *hmacKeyAddres) {
  u32 tls;
  Result ret;
  u32 *cmdBuf;
  ...
  cmdBuf[1] = programId;
  *(bool *)(cmdBuf + 2) = isOutbox;
  *(u32 **)(cmdBuf + 4) = bufferSize;
  cmdBuf[3] = messageIdSize;
  FUN_0010f5b8((int *)&cmdBuf,5,bufferAddress,(int)bufferSize);
  FUN_0010f5b8((int *)&cmdBuf,7,hmacKeyAddres,0x20);
  FUN_0010ce86((int *)&cmdBuf,9,messageIdAddress,messageIdSize);
  ret = SendSyncRequest:0x32(sessionHandle);
  if (-1 < ret) {
    ret = cmdBuf[1];
  }
